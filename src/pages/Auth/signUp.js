...
const SignUpPage = () => {
  ...
  const signUp = async (e) => {
    ...
    try{
       let user = await signup(user_info);
       console.log(user);
    }catch(err){
        console.log(err);
    }
  ...
  return (
    <>
      <div className=" text-black">
        {/* <div className=" p-4">
          <img src="/auth/logo.png" style={{ display: "flex" }}></img>
        </div> */}
        <div>
          <div className="grid grid-cols-1 lg:grid-cols-2" style={{ height: "100vh" }}>
            ...
            <div className="flex flex-col text-start gap-x-10 justify-end">
              <div className=" flex justify-start ml-2 mb-10">
                <div className=" login-wrapper gap-10">
                  ...
                  <div className="login-subtitle pt-8 flex justify-start items-center gap-3">
                      <span>Join Waitlist:</span>
                      <img src="/auth/wait_time.png"></img>
                  </div>
                  <Input title={"Name"} placeholder={"Ronnie_Fieg"} name={"user_name"} change={inputChanges} defaultValue={""} type={"text"} />
                  ...
                  <Button title={"Sign Up"} change={signUp} className={"mt-10 text-[20px]"} />
                  <p className="not-have-account pt-5 gap-3">
                    <span>Already have an account?</span>
                    <Link to={"/signIn"}>
                      <a onClick={() => setOpen(true)} className="to-sign-up cursor-pointer ml-2">Login</a>
                    </Link>
                  </p>
                </div>
              </div>
